        ...
        for course in course_task:
            ...
            while __point_index < len(point_list["points"]):
                ...
                try:
                    ...
                    __point_index += 1
                    continue
                # 章节任务统计(便于核查"有些视频没刷":是待处理、已满100%跳过、还是未满100%需重刷)
                _sk_v = (job_info or {}).get('skipped_passed_video', 0)
                _sk_o = (job_info or {}).get('skipped_passed_other', 0)
                _re = (job_info or {}).get('redone_video', 0)
                logger.info(f'章节任务统计[{point["title"]}]: 待处理 {len(jobs)} 个 | 已满100%跳过: 视频 {_sk_v} 个, 其它 {_sk_o} 个 | 未满100%需重刷: {_re} 个')
