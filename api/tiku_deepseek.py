...
class TikuDeepSeek(Tiku):
    ...
    def _build_prompt(self, q_info: dict, image_text: str = "", require_option_value: bool = False) -> str:
        ...
        elif q_type in ("completion", "unknown"):
            # 填空题/简答题:答案直接作为填写内容,必须给纯文本,不能有换行/Markdown/题号
            lines.append(
                "输出要求: 必须给出一个确定答案,即使不确定也请写出最可能的内容,严禁输出“无法确定”、“不清楚”或留空;"
                "本题为填空题或简答题,请只输出答案内容本身;"
                "不要输出“答案:”“解析”等前缀,不要输出题号,不要换行,不要使用Markdown、不要加引号;"
                "若题目中有多个空(如 ______ 和 ______),请按空的先后顺序依次给出每个空的答案,并用中文逗号“，”分隔(例如:普通教育，特殊教育);"
                "简答题请用一句简洁的话作答。"
            )
            if require_option_value:
                lines.append("你的上一次回答不符合要求,请严格按上述格式只输出答案内容,不要输出其他任何文字。")
        else:
            lines.append(
